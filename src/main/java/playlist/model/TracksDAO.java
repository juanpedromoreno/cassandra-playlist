...
public class TracksDAO extends CassandraData {
	...
	private TracksDAO(Row row) {
		...
		try {
			...
			starred = false; // If the field doesn't exist or is null we set it
								// to false
		}
	}
	...
	public TracksDAO(String artist, String track, String genre,
		...
		starred = false; // We never set this when adding a track, so leave this
							// one alone
	}
	...
	public static List<TracksDAO> listSongsByGenre(String genre, int num_tracks) {
		...
		//
		// TODO_done - Add a line here to automatically page the results 200
		// rows at a time
		//
	...
	public void star() {
		...
		PreparedStatement preparedStatement = getSession()
				.prepare(
						"UPDATE track_by_artist  USING TTL 30 SET starred = true where artist = ? and track = ? and track_id = ?");
		...
		preparedStatement = getSession()
				.prepare(
						"UPDATE track_by_genre  USING TTL 30 SET starred = true where genre = ? and artist = ? and track = ? and track_id = ?");
