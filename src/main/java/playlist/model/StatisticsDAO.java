 ...
 * 
 * Handles the collection of counter statistics for the application. It has
 * methods to increment and decrement counters in Cassandra, as well as return a
 * list of them
 * 
 */
...
public class StatisticsDAO extends CassandraData {
	 ...
	 * 
	 * @return - Return a List of Statistics objects. Each includes the counter
	 *         name and its value.
	 * 
	 */
	...
	public static List<StatisticsDAO> getStatistics() {
		...
		for (Row row : results) {
			statistics.add(new StatisticsDAO(row)); // Lets use column 0 since
	...
	public static void increment_counter(String counter_name) {
		...
		// TODO_done
		// TODO_done - add code the increment the given counter
		// TODO_done
		PreparedStatement preparedStatement = getSession().prepare(
				"UPDATE statistics SET "
						+ "counter_value = counter_value + 1 WHERE "
						+ "counter_name = ?");
